...
from interview.context.graph_reader import InterviewGraphReader
from interview.context.models import EquipmentContext, WorkContext
...
class ContextBuilder:
    """Turns a profile into a WorkContext, grounded in the graph. Bound to a
    reader so the graph dependency is injected, not reached for."""

    def __init__(self, reader: InterviewGraphReader):
        self._reader = reader

    async def build(self, profile: dict) -> WorkContext:
        # the neo4j driver is sync; keep the event loop free
        return await asyncio.to_thread(self._gather, profile)

    def _gather(self, profile: dict) -> WorkContext:
        reader = self._reader
        terms = self._search_terms(profile)
        tags = [r["tag"] for r in reader.equipment_matching(terms)
                if r.get("tag")][:8]

        # one round trip for all eight tags, not four per tag
        ctx_by_tag = reader.equipment_context(tags)
        equipment = [EquipmentContext(tag=tag, **ctx_by_tag[tag])
                     for tag in tags if tag in ctx_by_tag]
        ...
        ctx.brief = self._render_brief(ctx)
        return ctx

    @staticmethod
    def _search_terms(profile: dict) -> list:
        """Tag-shaped tokens plus meaningful words from projects/expertise."""
        ...
        if ctx.equipment:
            ...
            for eq in ctx.equipment:
                ...
                for f in eq.failures[:4]:
                    causes = ", ".join(f.get("causes") or []) \
                        or "causes not recorded"
                ...
                procs = [x.get("procedure") for x in eq.procedures
                         if x.get("procedure")]
                if procs:
            ...
            lines.append("- none found for this profile; interview from the"
                         " profile alone and ask them to name their key"
                         " equipment")
